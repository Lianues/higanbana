...
(() => {
  const G = globalThis;
  console.log('[Higanbana] Runtime script executing...');
  ...
  try {
    (function() {
      ...
      const _withZ = async (p) => {
        if (!p.zipArrayBuffer && p.zipBlob) {
          p.zipArrayBuffer = await _toBuf(p.zipBlob);
          try { delete p.zipBlob; } catch(e){}
        } else if (p.zipArrayBuffer) { p.zipArrayBuffer = await _toBuf(p.zipArrayBuffer); }
        return p;
      };
      const Api = {
        getProject: (p) => rpc('getProject', _withT(Object.assign({}, p))),
        createProject: async (p) => rpc('createProject', await _withZ(Object.assign({}, p))),
        updateProject: async (p) => {
          const payload = _withT(await _withZ(Object.assign({}, p)));
          if (payload.__hbAllowBroadcastZipTarget === undefined) payload.__hbAllowBroadcastZipTarget = !canPostToParent && !canPostToOpener;
          return rpc('updateProject', payload);
        },
        deleteProject: (p) => {
          const payload = _withT(Object.assign({}, p));
          if (payload.__hbAllowBroadcastZipTarget === undefined) payload.__hbAllowBroadcastZipTarget = !canPostToParent && !canPostToOpener;
          return rpc('deleteProject', payload);
        }
      };
      window.Higanbana = Api;
      window.higanbana = Api;
      G.__HB_HIGANBANA_API_READY__ = true;
      console.log('[Higanbana] API exposed in iframe', Api);
    })();
  } catch (e) {
...
<\/script>
`}function d(t,r={}){const e=String(t??"");if(!e.trim()||e.includes(s))return e;const n=l(r),i=/<head\b[^>]*>/i;if(i.test(e))return e.replace(i,o=>o+n);const a=/<html\b[^>]*>/i;return a.test(e)?e.replace(a,o=>o+`<head>${n}</head>`):n+e}export{c as H,d as i};
