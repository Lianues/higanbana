const c="hb_higanbana_html_bridge_v1",s="/*__HB_HTML_COMPAT__*/";function l(t){const a=String(t.origin??"").trim(),e=!!t.forceBaseHref;return`
<script>${s}
(() => {
  const G = globalThis;
  ...
  const INJECTED_ORIGIN = ${JSON.stringify(a)};
  ...
  try {
    ...
    try {
      console.warn('[Higanbana] inject runtime API registration failed', e);
    } catch {}
  }
...
<\/script>
`}function d(t,a={}){const e=String(t??"");if(!e.trim()||e.includes(s))return e;const r=l(a),o=/<head\b[^>]*>/i;if(o.test(e))return e.replace(o,n=>n+r);const i=/<html\b[^>]*>/i;return i.test(e)?e.replace(i,n=>n+`<head>${r}</head>`):e.toLowerCase().includes("<!doctype")?e.replace(/(<!doctype[^>]*>)/i,"$1"+r):r+e}export{c as H,d as i};
