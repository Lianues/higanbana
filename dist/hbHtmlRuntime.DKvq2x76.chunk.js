...
(() => {
  ...
  try {
    if (DIRECT_WIN && typeof DIRECT_WIN === 'object') {
      ...
      const installPassthrough = (k) => {
        try {
          const currentDesc = Object.getOwnPropertyDescriptor(G, k);
          // 优先装 getter 直连上层，避免“打开新标签页时 ST_API 尚未初始化”导致的时序丢失
      ...
      for (const k of passthroughKeys) {
        installPassthrough(k);
      }
    }

    // 兼容大小写别名（在 getter 透传后也保持可访问）
