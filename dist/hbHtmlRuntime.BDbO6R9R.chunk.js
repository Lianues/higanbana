const c="hb_higanbana_html_bridge_v1",s="/*__HB_HTML_COMPAT__*/";function f(t){const r=String(t.origin??"").trim(),e=!!t.forceBaseHref;return`
<script>${s}
(() => {
  const G = globalThis;
  ...
  try {
    ...
    // 兼容旧 API：内部统一转发到 updateProject
    if (typeof G.Higanbana.overwriteProject !== 'function') {
      G.Higanbana.overwriteProject = (params) => updateProject(params || {});
    }

    if (typeof G.Higanbana.overwriteCurrentProject !== 'function') {
      G.Higanbana.overwriteCurrentProject = (params) => updateProject(params || {});
    }

    const asLegacyImportPayload = (params = {}) => {
      const p = params && typeof params === 'object' ? Object.assign({}, params) : {};
      const importZipBase64 = String(p.importZipBase64 || '').trim();
      const legacyZipBase64 = String(p.zipBase64 || '').trim();
      if (!importZipBase64 && legacyZipBase64) {
        p.importZipBase64 = legacyZipBase64;
      }
      return p;
    };

    if (typeof G.Higanbana.importZipAndOverwriteProject !== 'function') {
      G.Higanbana.importZipAndOverwriteProject = (params) => updateProject(asLegacyImportPayload(params || {}));
    }

    if (typeof G.Higanbana.importZipAndOverwriteCurrentProject !== 'function') {
      G.Higanbana.importZipAndOverwriteCurrentProject = (params) => updateProject(asLegacyImportPayload(params || {}));
    }
...
<\/script>
`}function l(t,r={}){const e=String(t??"");if(!e.trim()||e.includes(s))return e;const n=f(r),o=/<head\\b[^>]*>/i;if(o.test(e))return e.replace(o,a=>a+n);const i=/<html\\b[^>]*>/i;return i.test(e)?e.replace(i,a=>a+`<head>${n}</head>`):n+e}export{c as H,l as i};
