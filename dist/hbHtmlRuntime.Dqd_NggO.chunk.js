...
(() => {
  ...
  if (rawFetch) {
    G.fetch = async (input, init) => {
      ...
      // 仅对非 GET/HEAD/OPTIONS 注入 CSRF（更贴近酒馆行为）
      const method = String((init && init.method) || (isReq ? input.method : 'GET') || 'GET').toUpperCase();
      const needToken = method !== 'GET' && method !== 'HEAD' && method !== 'OPTIONS';
      ...
      if (isSameOrigin && needToken && !hasToken) {
        try {
  ...
  try {
    ...
    if (XHR && XHR.prototype && !XHR.prototype.__hbPatched) {
      ...
      XHR.prototype.send = function(body) {
        ...
        if (!this.__hbAsync) {
          try {
            if (isSameOrigin && needToken && !hasToken && csrfToken) {
