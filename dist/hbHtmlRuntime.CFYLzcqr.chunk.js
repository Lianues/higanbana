...
(() => {
  ...
  // ---- 提供 Higanbana 项目管理 API（供项目页面自管理） ----
  try {
    ...
    const withCurrentTargetFallback = (payload) => {
    ...
    let hbApi = null;
    try {
      const existing = G.Higanbana;
      if (existing && typeof existing === 'object') hbApi = existing;
    } catch {
      // ignore
    }
    ...
    if (!hbApi) {
      try {
        G.Higanbana = {};
        if (G.Higanbana && typeof G.Higanbana === 'object') hbApi = G.Higanbana;
      } catch {
        // ignore
      }
    }
    ...
    if (!hbApi) {
      try {
        const boxed = {};
        Object.defineProperty(G, 'Higanbana', { value: boxed, configurable: true, writable: true });
        hbApi = boxed;
      } catch {
        // ignore
      }
    ...
    if (!hbApi || typeof hbApi !== 'object') {
      throw new Error('无法挂载 window.Higanbana（全局对象不可写或被同名属性占用）');
    }

    const defineFn = (name, fn) => {
      try {
        if (typeof hbApi[name] === 'function') return;
      } catch {
        // ignore
      }
      try {
        ...
        try {
          Object.defineProperty(hbApi, name, { value: fn, configurable: true, writable: true });
        } catch {
          // ignore
        }
      }
    ...
    defineFn('updateProject', (params) => updateProject(params || {}));
    defineFn('createProject', (params) => createProject(params || {}));
    defineFn('deleteProject', (params) => deleteProject(params || {}));
    defineFn('getProject', (params) => getProject(params || {}));
    ...
    try {
      ...
      try {
        Object.defineProperty(G, 'higanbana', { value: hbApi, configurable: true, writable: true });
      } catch {
        // ignore
      }
    }
    ...
    try {
      ...
      G.__HB_HIGANBANA_API_ERROR__ = e && e.message ? String(e.message) : String(e);
      console.warn('[Higanbana] expose runtime API failed', e);
    } catch {
      // ignore
    }
  }
...
<\/script>
`}function f(t,r={}){const e=String(t??"");if(!e.trim()||e.includes(s))return e;const a=l(r),o=/<head\\b[^>]*>/i;if(o.test(e))return e.replace(o,n=>n+a);const i=/<html\\b[^>]*>/i;return i.test(e)?e.replace(i,n=>n+`<head>${a}</head>`):a+e}export{c as H,f as i};
